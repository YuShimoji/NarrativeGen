...
function renderSaveSlots() {
  ...
  for (let i = 0; i < SAVE_SLOTS; i++) {
    const slotId = `${SAVE_KEY_PREFIX}${i}`
    const slotName = localStorage.getItem(slotId)
    saveSlotsHtml += `<button class="save-slot-btn" data-slot-id="${slotId}">${slotName || `スロット${i + 1}`}</button>`
  }
  saveSlots.innerHTML = saveSlotsHtml

  // Add event listeners for save slots
  const saveSlotBtns = saveSlots.children
  for (const btn of saveSlotBtns) {
    btn.addEventListener('click', () => {
      const slotId = btn.dataset.slotId
      const slotName = prompt('セーブ名を入力してください:')
      if (slotName) {
        localStorage.setItem(slotId, slotName)
        localStorage.setItem(`${slotId}_data`, JSON.stringify(appState.model))
        renderSaveSlots()
      }
    })
  }
...
function startAutoSave() {
  const autoSaveInterval = setInterval(() => {
    const modelName = getCurrentModelName()
    if (modelName) {
      localStorage.setItem(AUTOSAVE_KEY, JSON.stringify(appState.model))
    }
  }, 30000) // 30 seconds
}
...
>>>>>>> 9d0d181 (Refactor main.js: Remove legacy duplicate session management block and unify to core/session.js API)
=======
﻿// Handle potential IDE extension conflicts (e.g., migrationWizard.js errors)
try {
  // Check if we're in an IDE environment that might have conflicting scripts
...
>>>>>>> 3e73d0604b8100b65471654dce4844707624ef02
