...
1. **ライターのペルソナ**: 技術者か非技術者か? DSLに抵抗はあるか?
2. **執筆量**: 一つのモデルに何ノード程度を想定するか?
3. **条件/効果の複雑さ**: ライター自身が設定するか、別の担当者が付与するか?
4. **WritingPage側の変更許容度**: NarrativeGen連携のためにWritingPageを改修する余地はあるか?
5. **UnityChatNovelGame連携の優先度**: Yarn出力が今すぐ必要か、将来の話か?
6. **パイプラインの最終形**: 「書く → 構造化 → テスト → 出力 → ゲームに組み込み」の各段階でどのツールが担うか?

---

## 推奨される次のステップ

1. 上記の「未解決の問い」に対するオーナー判断
2. 最も有望な1方向を選択し、最小限のPoCを実施
3. PoC結果をもとに本格設計を確定
