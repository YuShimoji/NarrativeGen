...
**所要時間**: 30-45 分 (Stage 2 チュートリアル部分)
...
NarrativeGen での物語制作は5つのステージで進む。
...
- **Stage 1**: ツールの外でストーリーの骨格を設計する
- **Stage 2**: Web Tester でノード・選択肢・条件・効果を作り込む (このガイドの主要部分)
- **Stage 3**: バリデーションツールと推論エンジンで品質を検証し、プレイテストする
- **Stage 4**: JSON / CSV / Yarn / Ink / Twine にエクスポートする
- **Stage 5**: Unity や他のランタイムにモデルを統合する

通常は1人で全ステージを担当する。「ライター」「テクニカルデザイナー」「インテグレーター」は帽子の切替として理解すればよい。

---

## Stage 1: 構想・設計 (Design)

Web Tester を開く前に、ストーリーの骨格を決めておくと制作がスムーズに進む。この段階は完全に手作業であり、ツールは任意で使う。

### 何を決めるか

1. **ストーリー構造 (ビート表)**: 物語の主要な場面転換を箇条書きにする。各ビートが後のノードになる
2. **キャラクター**: 登場人物の名前・役割・知識領域を決める。Stage 2 でエンティティやキャラクター知識モデルに反映する
3. **世界設定**: 場所・アイテム・組織など、物語に登場する「もの」を列挙する。Stage 2 でエンティティとして定義する
4. **分岐ロジック**: どの選択がどの結末に繋がるかの概略図を描く。フラグ・リソース・変数の候補もここで洗い出す
5. **テンプレート / 会話パターン**: 条件に応じて自動挿入したい台詞パターンがあれば、ここでメモしておく

### 推奨する準備物

- ビート表 (テキストファイルやホワイトボード)
- キャラクター表 (名前、関連する知識領域、主な行動)
- エンティティ候補リスト (名前、プロパティ候補)
- 分岐概略図 (手描きでもよい。Web Tester の NodeGraph タブで後から可視化できる)

### チェックリスト: Stage 2 に進む前に

- [ ] 主要な場面 (ノード) の名前と順序が決まっている
- [ ] 主要な分岐と条件の概要が決まっている
- [ ] 登場人物とアイテムの候補リストがある
- [ ] エンディング (到達目標) が少なくとも1つ定義されている
...
以下のセクション 1-14 は Stage 2 (コンテンツ制作) の詳細チュートリアルである。
...
Stage 2 完了後:
- [Stage 3: 検証・調整](#stage-3-検証調整-validate)
- [Stage 4: エクスポート](#stage-4-エクスポート-export)
- [Stage 5: ランタイム統合](#stage-5-ランタイム統合-integrate)
...
Stage 2 でモデルを作り終えたら、エクスポートする前に検証を行う。NarrativeGen は自動検証ツールと手動確認を組み合わせて品質を担保する。

### スキーマ検証

コマンドラインで全モデルを一括検証できる:

```bash
npm run validate
# → "Validated: your_model.json" が表示されればOK
```

スキーマ検証は JSON 構造の妥当性を確認する (必須フィールド、型、参照の整合性)。

### 構造検証 (Validation Panel)

Web Tester の Validation Panel では以下が自動チェックされる:

- **循環参照**: 選択肢が自分自身のノードに戻るループ
- **孤立ノード**: どこからも到達できないノード
- **未定義参照**: 存在しないノードIDへの遷移
- **未使用フラグ/リソース**: 定義されているが条件/効果で参照されていない変数

### 推論エンジンによる分析

NodeGraph タブで推論エンジンを使った高度な分析ができる:

| 機能 | 用途 | 操作 |
|------|------|------|
| パス分析 (UC-1) | 開始から各エンディングまでの全ルートを列挙 | ノード右クリック → "パス分析" |
| 到達可能性 (UC-2) | 指定ノードに到達可能かを条件付きで検証 | ノード右クリック → "到達可能性" |
| 影響分析 (UC-3) | ある条件/効果が他のノードにどう波及するかを表示 | 推論パネル → "影響分析" |
| What-if (UC-5) | 仮の状態を設定してシナリオを試す | 推論パネル → "What-if" |

推論ハイライトにより、分析結果がグラフ上に色分け表示される。

### プレイテスト

Story タブの Play モードで実際に物語を通しプレイする。確認すべきポイント:

- 全主要ルートを少なくとも1回は通過する
- 条件による選択肢の出現/非表示が意図通りか
- Dynamic Text の展開が正しいか
- テキストの読み心地 (段落フェードイン、選択肢のインライン配置)
- 画像/BGM を設定した場合はその再生確認

### チェックリスト: Stage 4 に進む前に

- [ ] `npm run validate` が通過する
- [ ] Validation Panel で構造エラーがゼロ
- [ ] 到達可能性分析で孤立ノードがゼロ
- [ ] 主要ルートのプレイテストが完了
- [ ] Dynamic Text / 条件分岐の動作を確認済み

---

## Stage 4: エクスポート (Export)

検証済みモデルをターゲットランタイム用にエクスポートする。

### エクスポート形式

| 形式 | 用途 | 特徴 |
|------|------|------|
| **JSON** | NarrativeGen engine-ts / sdk-unity | **主要パス**。全機能が完全に保存される |
| CSV | スプレッドシート編集 / 外部ツール連携 | テキスト一括編集に便利。構造は単純化される |
| Yarn | Unity Yarn Spinner ランタイム | Unity 連携の互換形式 |
| Ink | Inkle ランタイム | Inkle エコシステムとの互換 |
| Twine | Twine ランタイム | Twine コミュニティとの互換 |

ツールバーの「エクスポート...」から形式を選択してダウンロード。

### JSON 主軸戦略

NarrativeGen の独自機能 (Dynamic Text、Entity-Property、ConversationTemplate、Event、Character Knowledge) は **JSON 形式でのみ完全に保存される**。Yarn / Ink / Twine は互換用であり、これらの独自機能は部分的にしかエクスポートされない。

独自機能を活用する場合は JSON エクスポート + engine-ts または sdk-unity ランタイムを使用すること。

### 制限事項

- Dynamic Text 構文 (`[entity.property]`、`{?condition:text}`) は Yarn / Ink / Twine で非互換。プレーンテキストとして出力される
- Entity-Property / ConversationTemplate / Event のメタデータは JSON 以外では失われる
- CSV インポート → 編集 → CSV エクスポートのラウンドトリップは可能だが、条件/効果の複雑な構造は単純化される

---

## Stage 5: ランタイム統合 (Integrate)

エクスポートしたモデルを Unity や他のランタイムに組み込む。

### Unity 統合 (JSON + sdk-unity)

1. **モデル配置**: エクスポートした JSON ファイルを Unity プロジェクトの `Assets/Resources/` 等に配置
2. **SDK セットアップ**: Unity Package Manager から NarrativeGen SDK をインストール (UPM)
3. **UI 接続**: テキスト表示コンポーネントと選択肢ボタンを SDK の API に接続
...
- **Yarn Spinner**: `.yarn` ファイルを Unity Yarn Spinner ランタイムで読み込む
- **Ink**: `.ink` ファイルを Inkle ランタイムで読み込む
- **Twine**: `.html` ファイルをブラウザで直接開く

これらの形式では NarrativeGen 独自機能は利用できない。

---

## 高度な機能

このガイドでカバーした機能を使いこなせたら、以下の高度な機能も活用できる:

- **ノードグループ**: `node_group` フィールドでノードを階層化する
- **プロパティ異常検出**: エンティティのプロパティ値が期待範囲を超えたときの検出
- **説明トラッカー**: 同じエンティティを繰り返し説明しないための制御 (`[entity~]` 構文)
