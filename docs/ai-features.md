...
NarrativeGen の言い換えは「非AIの決定的（deterministic）なバリアント生成」を中核とし、ライター/デザイナーが安全に文体統一・表現バリエーションを作るための支援機能です。AI 連携（OpenAI 等）は任意の追加オプションであり、プロジェクトや開発をブロックしません。
...
- ~~Phase 3: ローカル LLM 統合（Ollama）~~ → 削除決定（2026-03-07、精度・リソース負荷の観点）
- **Phase 3**: 生成履歴・再生成機能
...
- 現在: Mock（テスト用）+ OpenAI（クラウドAPI）
- Ollama: 2026-03-07に削除（精度・リソース負荷の問題）
