...
5. ワーカー数: 単一の Vite dev サーバに複数ワーカーが同時にセッションを掛けると、`appState.model` の待機が数十秒〜タイムアウトまでブロックすることがある。`apps/web-tester/playwright.config.js` では **workers を 1 に固定**している（ローカル・CI 共通）。
...
- 起票済み・**クローズ済み（2026-04-09 対策コミット）**:
  - [#81](https://github.com/YuShimoji/NarrativeGen/issues/81) FLK-PLAY-AC5 — `play-immersion.spec.js` 待機強化
  - [#82](https://github.com/YuShimoji/NarrativeGen/issues/82) FLK-UNDO-GRAPH — `undo-redo.spec.js` シリアル + グラフ SVG 待ち
  - [#83](https://github.com/YuShimoji/NarrativeGen/issues/83) FLK-MODAL-FOCUS — `modal-a11y.spec.js` 追加
