# 申し送りドキュメント 2025-12-06 (更新: 2025-12-12)

## 2025-12-12 追加作業

- ドキュメント同期: `docs/PROJECT_STATUS.md`, `docs/TECHNICAL_DEBT.md`, `features-status.md`
- 申し送り整形: `docs/HANDOVER_2025-12-06.md` の表記ゆれ/表の崩れ（`|` 混入）を修正
- markdownlint 対応: 見出し/リスト/フェンス周りの空白調整で全ドキュメントの lint 警告解消
- 末尾空白調査: git diff --check で検知された末尾空白は、CRLF/LF の混合と既存の markdownlint 設定によるものと特定
- 作業ツリー確認: リモート同期済み、変更ファイル4つ（ドキュメントのみ）でクリーン状態
- 次フェーズ準備: GUI エディタ手動テスト（`GUI_EDITOR_TEST_GUIDE.md`）再開準備完了

### 現在の作業状態
- **完了**: ドキュメント同期・lint 修正・リモート同期確認
- **次作業**: GUI エディタ機能の手動テスト一式（コピー＆ペースト、検索・フィルタ、スニペット、テンプレート、リアルタイムプレビュー、モデル検証、Mermaid プレビュー）
- **保留**: UI 改善テスト（`UI_IMPROVEMENTS_TEST.md`）、Phase 2 グラフエディタ決定

### 技術調査結果
- markdownlint の末尾空白検知は、Windows (CRLF) とプロジェクト設定の相性によるもの
- 機能的には問題ないが、コミット前に `git diff --check` で確認済み
- Web Tester のレキシコン拡張（モデル埋め込み）は実装済みで、スキーマ定義も完了

### 引き継ぎポイント
- 開発環境: Windows + Node.js + Vite 開発サーバー
- ローカル変更: ドキュメント4ファイルのみ（機能コード変更なし）
- テスト環境: `http://localhost:5173` で Web Tester 起動可能
- 次作業者へ: GUI エディタテストから再開、テスト結果を本申し送りまたは別レポートに記録

## 2025-12-11 追加作業

### 1. レキシコン拡張（非UI）と仕様同期

- モデル JSON の `meta.paraphraseLexicon` を Web Tester 起動時に取り込む処理を実装
  - `apps/web-tester/main.js`: `applyModelParaphraseLexicon(model)` を追加し、サンプル読込・ファイル読込・ドラッグ＆ドロップ・`safeStartSession` など、モデル読み込み経路すべてで呼び出し
  - エンジン側レキシコン（`setParaphraseLexicon`）および Web Tester の `LexiconManager` にマージ（localStorage には保存しない）
- デザイナー辞書 UI に影響を与えず、「モデル同梱レキシコン」をセッション単位で利用可能に

### 2. レキシコン JSON スキーマの追加

- `Packages/engine-ts/schemas/lexicon.schema.json` を追加
  - `docs/reference.md` で提案していたスキーマ案をそのまま JSON Schema として配置
  - 現時点ではランタイムから未使用（将来のインポート時バリデーション・エディタ補完向け）

### 3. ドキュメント更新・現状整理

- `docs/reference.md` のレキシコン節を最新実装に合わせて更新
  - 3.2「レキシコン拡張」を「部分実装・提案」として整理
  - 3.2.2 モデル埋め込みレキシコン: Web Tester での `meta.paraphraseLexicon` 取り込みが実装済みである旨を明記
  - 3.2.3 レキシコン JSON スキーマ: 実在するスキーマファイルのパスと、バリデーション処理が未実装であることを明記
- これにより、「仕様ドキュメント上は提案だがコードは実装済み」というギャップを解消

### 4. 手動テスト状況の確認

- `branching_flags.json` を用いた Save/Load 基本ケースを手動で確認
  - GUI 編集モードでの編集 → 保存 → 再読み込みが期待どおり動作することを確認
  - 現時点ではその他の GUI テスト（スニペット/テンプレート/検索・フィルタ等）はユーザー判断により後回しとし、今後 B フェーズでまとめて実施予定
...
| Save/Load | ✅ 基本ケース実施 | `branching_flags.json` を用いた確認済み |
