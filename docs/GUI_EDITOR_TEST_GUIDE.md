...
### TC-EN-01: パネルの折りたたみ/展開

- **手順**:
  1. GUI編集モードに入る
  2. ノードリスト上部の「エンティティ定義 (0)」ヘッダーを確認
  3. ヘッダーをクリックして展開する
  4. 再度クリックして折りたたむ
- **期待結果**:
  - デフォルトは折りたたみ状態
  - ヘッダークリックで展開/折りたたみが切り替わる
  - 三角アイコンが回転する (右向き=閉じ、下向き=開き)
  - キーボード (Enter/Space) でも操作できる
...
- **手順**:
  1. パネルを展開する
  2. 「+ エンティティ追加」ボタンをクリック
  3. 新しい行がテーブルに追加されることを確認
- **期待結果**:
  - ID `item_1` (または連番)、名前「アイテム1」、コスト 0 の行が追加される
  - パネルが折りたたみ状態だった場合、自動的に展開される
  - ヘッダーのカウント表示 (0) → (1) に更新される

### TC-EN-03: エンティティのインライン編集

- **手順**:
  1. エンティティが1件以上ある状態で、名前フィールドをクリック
  2. 名前を変更してフォーカスを外す (Tab キー、または別のフィールドをクリック)
  3. 説明、コストについても同様に編集
- **期待結果**:
  - 各フィールドがインライン編集可能
  - フォーカスを外した時点で `change` イベントが発火し、モデルに反映される
  - コストフィールドは数値入力のみ受け付ける

### TC-EN-04: エンティティIDの変更

- **手順**:
  1. 既存エンティティのIDフィールドを編集し、新しいIDに変更
  2. フォーカスを外す
- **期待結果**:
  - IDが変更され、テーブルが再レンダリングされる
  - 既存のIDと重複する場合はステータスバーに警告が表示され、元のIDに戻る
...
- **期待結果**:
  - 該当行がテーブルから削除される
  - カウント表示が更新される
  - 全て削除すると「エンティティが定義されていません」メッセージが表示される

### TC-EN-06: 保存と読み込み

- **手順**:
  1. エンティティを2〜3件追加し、各フィールドを編集
  2. 「保存して適用」ボタンをクリック
  3. 「JSON保存」ボタンでファイルをダウンロード
  4. ダウンロードしたJSONファイルを開き、`entities` フィールドを確認
  5. そのJSONファイルを再度読み込み、GUI編集モードに入る
- **期待結果**:
  - JSONファイルの `entities` オブジェクトにエンティティ定義が含まれている
  - 再読み込み後、エンティティパネルに同じ内容が表示される

### TC-EN-07: hasItem条件との連携

- **手順**:
  1. エンティティを1件追加 (例: ID `sword`)
  2. 任意のノードの選択肢を開く
  3. 条件に「アイテム所持」を追加し、名前に `sword` を入力
  4. 別の選択肢の効果に「アイテム追加」を追加し、名前に `sword` を入力
  5. 「保存して適用」し、プレイモードで確認
- **期待結果**:
  - アイテム追加効果が発動するまで、hasItem条件付き選択肢は非表示
  - アイテム追加後、条件付き選択肢が表示される
...
| 2026-03-17 | エンティティ定義管理パネル (TC-EN-01〜07) 追加 |
