...
| （人的確認待ち） | 担当者記入 | ブラウザ名 / OS | Pass/Fail | Pass/Fail | Pass/Fail | Pass/Fail | 記入後は `docs/TECHNICAL_DEBT.md` の該当項目を更新 |

### 人的確認手順（AC-9〜12）

1. `npm run dev` で Web Tester を起動し、BGM 付きサンプルを読み込む。  
2. 同一タブで以下を確認して表に記録する。  
   - AC-9: 指定ノード遷移で BGM が再生される  
   - AC-10: BGM 切替時にクロスフェード時間が設定値に概ね一致する  
   - AC-11: `bgm: null` ノードでフェードアウト停止する  
   - AC-12: 初回操作前にエラーが出ず、初回ユーザー操作後に再生開始する  
3. ブラウザ別（最低 Chrome / Edge）で同様に確認する。  
4. Fail があれば発生条件（ノードID、遷移元/先、再現率）を備考欄へ記載する。
