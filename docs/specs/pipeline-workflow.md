...
## HUMAN_AUTHORITY 確認事項

以下は方向性の確認が必要な項目:

1. **ペルソナの妥当性**: P1/P2/P3 の分類は実態に合っているか? 実際には1人で全ステージを担当する想定か?
2. **WritingPage 連携の優先度**: Stage 2a をどの時点で着手するか
3. **エクスポート戦略**: JSON + engine-ts ランタイムを主軸とし、他形式は補助とする方針でよいか
4. **Stage 1 の AI 支援**: ストーリー構造提案をスコープに含めるか
5. **Pipeline ガイドの形態**: AUTHORING_GUIDE.md の拡張か、独立ドキュメントか
