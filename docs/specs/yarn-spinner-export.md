...
## Dynamic Text 変換ポリシー（2026-04）

Yarn エクスポート時の Dynamic Text は次の方針で扱う。

- 変換対象
  - `{variable}` は `$variable` として出力（文脈が条件式なら Yarn 条件式として扱う）
- 非対応
  - `[entity.property]` / `[entity~]` / 会話テンプレート挿入依存表現
- フォールバック
  - 非対応構文は原文のまま保持する
  - エクスポート結果における「意味の変化」を避けるため、推測変換は行わない

この方針は `docs/specs/dynamic-text-engine.md` の「エクスポート方針（2026-04 確定）」と同期する。
