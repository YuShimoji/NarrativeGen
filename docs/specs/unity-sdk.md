...
**Status**: partial | **Pct**: 98 | **Cat**: system
...
| 機能 | TS | C# | 状態 |
|------|----|----|------|
| 条件: flag, resource, variable, hasItem, hasEvent, timeWindow | 6種基本 | 6種基本 | 完了 |
...
| `resolveNarrativeDisplayTextTracked` | `narrative-display-text.ts` | `NarrativeDisplayText.cs` | **対応**（`maxUses` ランタイム消費は未） |
...
- [ ] TS とのエッジケース完全一致（数値型の微妙な差・`maxUses` 消費など）
- [ ] NuGet パッケージ配布
