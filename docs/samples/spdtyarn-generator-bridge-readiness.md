...
- classification: `deterministic_bridge_adapter_implemented`
- reason: SP-DTYARN still has a runnable `YarnFormatter` export path, and the repo now also has a separate deterministic bridge adapter that consumes `StoryContextPacket` and returns a `StructuredContinuationProposal`.
...
| `packages/engine-ts/test/spdtyarn-bridge-adapter.spec.ts` | Focused adapter test | runnable through `npm run test -w @narrativegen/engine-ts` | Proves the adapter reads current node text/id, route, resources, variables, story pressure, and emits choice/effect proposal. |
