...
| 機能 | 状態 | 備考 |
|------|------|------|
| タブレイアウト (ストーリー/ノードグラフ/デバッグ/リファレンス/アドバンスド) | 実装済 | クリックイベント実装済 (`GUI_EDITOR_TEST_GUIDE.md` 参照) |
...
| AI プロバイダ設定 (Mock/OpenAI) | 実装済(要実環境テスト) | APIキー設定 UI あり（Ollama削除: 2026-03-07） |
| AI による次ノード案生成・言い換え | 実装済(要実環境テスト) | 失敗時はエラーメッセージ + ログ |
| キーバインド (s/g/d/a など) | 実装済 | 設定 UI + localStorage 保存。重複キー検証あり |
| 高度なショートカット (Ctrl+Z 等) | 実装済 | グラフエディタでUndo/Redo対応 |
...
これらは Phase 2 で実装済み（GraphEditorManager, ValidationPanel, SearchManager）。
...
- **低優先度**
  - ~~高度なショートカット (Undo/Redo 等)~~ (実装済み: グラフエディタで Undo/Redo 対応)
  - バージョン管理・履歴表示との連携
...
詳細なテスト手順は `docs/GUI_EDITOR_TEST_GUIDE.md` を参照。ここでは Web Tester 全体としての受け入れ基準を簡潔にまとめる。
