...
1. **ノードIDリネーム + 参照更新**
   ...
   - 現状: 手動で ID を変えると参照切れの危険がある
   - 対応方針: `GuiEditorManager` に統合し、1操作で ID + 参照を安全に書き換える

2. **ノード/選択肢 DnD 並べ替え**
   - 影響範囲: ノード一覧 (`metadata.nodeOrder`)、同一ノード内の選択肢順
   - 現状: 旧 handlers 実装にのみ存在、現行 GUI には未搭載
   - 対応方針: 軽量な DnD ライブラリ or ネイティブ DnD を用いた並べ替え実装

3. **条件/効果/アウトカムの構造化 GUI 編集**
   - 現状: 手入力文字列 or JSON での編集が中心
   - 対応方針: 型安全なフォーム (条件種別ごとの入力欄) を段階的に導入
