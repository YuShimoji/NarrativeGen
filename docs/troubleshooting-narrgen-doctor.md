...
- テスト環境の準備可能性（`docs/GUI_EDITOR_TEST_GUIDE.md` の前提条件）
...
**復旧手順**:
1. `docs/GUI_EDITOR_TEST_GUIDE.md` で必要なNode.jsバージョンを確認
2. Node.jsを更新:
...
#### 警告: `docs/GUI_EDITOR_TEST_GUIDE.md not found`

**原因**: テスト手順ドキュメントが存在しない

**復旧手順**:
1. `docs/GUI_EDITOR_TEST_GUIDE.md` が存在することを確認
2. 存在しない場合、Gitから取得:
   ```bash
   git checkout docs/GUI_EDITOR_TEST_GUIDE.md
   ```
...
2. `docs/GUI_EDITOR_TEST_GUIDE.md` の要件を満たしているか確認
3. 必要に応じて`.github/workflows/`の設定を更新

## 関連ドキュメント

- `docs/GUI_EDITOR_TEST_GUIDE.md` - GUI 編集系テスト環境の要件
