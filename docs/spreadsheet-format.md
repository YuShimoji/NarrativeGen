...
`timeWindow:<start>-<end>` は、現在の時間が指定された範囲内にあるかを判定します。

**評価ロジック**: `time >= start && time <= end`（両端を含む）

- **境界の扱い**: 開始時間と終了時間の**両端を含む**（inclusive）
- **例**: `timeWindow:5-10` は、現在の時間が **5以上10以下** の場合に条件を満たします
  - 時間5: ✅ 条件を満たす
  - 時間7: ✅ 条件を満たす
  - 時間10: ✅ 条件を満たす
  - 時間4: ❌ 条件を満たさない
...
**説明**:
- `timeWindow:5-10` 条件により、「市場に行く」選択肢は現在の時間が5以上10以下（両端を含む）の場合にのみ表示されます
- 時間5、6、7、8、9、10のいずれでも選択肢が表示され、時間4や11では表示されません
