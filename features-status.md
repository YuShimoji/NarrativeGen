...
| カテゴリ | 機能名 | 優先度 | 説明 | 見込み時期 |
|---------|--------|--------|------|-----------|
| **Phase 2** | グラフエディタ | 高 | Mermaid風のビジュアルノード編集 | Phase 2 |
| | Dagre.js 統合 | 高 | 自動レイアウトアルゴリズム | Phase 2 |
| | ミニマップ | 中 | 大規模グラフの全体把握 | Phase 2 |

| カテゴリ | 機能名 | 優先度 | 説明 | 見込み時期 |
|---------|--------|--------|------|-----------|
| **レキシコン拡張** | GUI クイック追加 | 中 | GUI から直接レキシコン追加 | 次リリース |
| | モデル埋め込み | 中 | `meta.paraphraseLexicon` 対応 | 次リリース |
| | JSON スキーマ | 低 | レキシコン検証用スキーマ | 将来 |
...
**現在のフォーカス**: リファクタリング完了・レイアウト問題対応・テスト完了
**次のマイルストーン**: Phase 2 グラフエディタ / レキシコン拡張
